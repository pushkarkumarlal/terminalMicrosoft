...
constexpr bool isActionableFromGround(const wchar_t wch) noexcept
{
    ...
    // It's written like this to get MSVC to emit optimal assembly for findNextVTSequence.
...
static size_t findActionableFromGround(const wchar_t* data, size_t count, size_t i) noexcept
{
    auto p = data + i;

    // The following vectorized code replicates isActionableFromGround which is equivalent to
    //   (wch <= 0x1f) || (wch >= 0x7f && wch <= 0x9f)
    // or rather it's more machine friendly equivalent
    ...
    for (const auto end = data + count; p < end && !isActionableFromGround(*p); ++p)
    {
    }
    return p - data;
}
...
#pragma warning(suppress : 26438) // Avoid 'goto'(es .76).
    ...
    while (i < string.size())
    {
        {
            _runOffset = i;
            i = findActionableFromGround(string.data(), string.size(), i);
